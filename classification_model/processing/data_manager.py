...
def pre_pipeline_preparation(*, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # Handle NaN values (e.g., fill with 0 or the median age)
    data["age"] = data["age"].fillna(data["age"].median())

    # Round the float values and convert to integers
    data["age"] = data["age"].round().astype(int)

    # drop unnecessary variables
    data.drop(labels=config.classification_model_configuration.unused_fields, axis=1, inplace=True)
...
def load_pipeline(*, file_name: str) -> Pipeline:
    """Load a persisted pipeline from a file."""
